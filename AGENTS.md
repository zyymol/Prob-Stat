...
| D-Study | 1-1.5h | **教师**：前段讲解知识点，等用户确认"懂了"后出 2-3 道分层题 | 听讲、提问、确认理解 | 无（AI 可读 wiki 作为上下文，但不写入） |
| D-Practice | 1.5-2.5h | **沉默**：不提示，不干预 | 独立做题，产出解答 | 无 |
| D-Verify | 0.5h | **阅卷官**：逐题批改，给出错误分析 + 正确解法 → 留 5-10 min 让用户消化 → 用户用一句话总结"今天最大的教训" | 理解错误，总结教训 | 无 |
| D-Refactor | 0.5h | **编译器**：ingest 用户笔记，更新 wiki | 将理解和错题写入 raw/notes/ | **ingest 触发点** |
...
1. AI 给出逐题错误分析 + 正确解法
2. 留 5-10 分钟让用户对照理解
3. 用户用一句话总结"今天最大的教训是什么"
4. 这句话作为 D-Refactor ingest 的核心素材

**关键约束：**
- D-Study 阶段，AI 讲解的知识点来自教材和考纲，但这些素材本身**不被 ingest**。
- D-Practice 阶段，AI 必须保持沉默。
- D-Verify 的批改结果不直接写入 wiki，而是由用户在 D-Refactor 消化后写入自己的笔记。
...
| D-Study | 教师（讲解→确认→出题） | ✅ 读概念页了解用户水平 | ❌ |
| D-Practice | 沉默 | ❌ | ❌ |
| D-Verify | 阅卷官（批改→留时间消化） | ❌ | ❌ |
